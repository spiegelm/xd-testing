...
function MultiDevice(options) {
     ...
     * @returns {boolean}
     */
    this.abortWaitCallback = () => this.abortWait
}
                        ...
                        if (addressOptions.implicit || addressOptions.any) {
                            ...
                            unifiedPromise = q.any(promises)
                                .then(() => {
                                    ...
                                    multiDevice.abortWait = true
                                    console.log("multiDevice.abortWait", multiDevice.abortWait)
                                })
                                ...
                                .then(() => q.all(promises)
                                    ...
                                    .finally(() => {
                                        console.log("finally")
                                        console.log("multiDevice.abortWait", multiDevice.abortWait)
                                        multiDevice.abortWait = false
                                        console.log("multiDevice.abortWait = false", multiDevice.abortWait)
                                    })
                                )
                        }
                    }
...
MultiDevice.prototype.getInstanceModifier = function() {
    ...
    return function(client) {
        ...
        client.getAbortWait = function() {
            return globalMultiDevice.abortWaitCallback()
        }
