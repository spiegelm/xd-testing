    ...
    return function (client) {
        ...
        client.next = function() {
            ...
            return this.lastPromise.done(function() {
                ...
                if (addressOptions.implicit) {
                    // Implicit device selection

                    // Determine relevant commands
                    if (fnName == "click") {

                        // TODO try using client.selectByElement

                        let fnArgs = args[args.length - 1] || []
                        let selector = fnArgs[0]

                        if (typeof selector !== "string") {
                            throw new Error('selector needs to be typeof "string"', 'ImplicitDeviceSelectionError')
                        }

                        // Async call, determine matching devices
                        intermediatePromise = q.all(deviceIds.map(id => multiDevice.instances[id])
                    ...
                    console.log("explicit command", fnName)
                ...
                return intermediatePromise.then(() => {
                    ...
                    usedDeviceIds.forEach(id => {
                        ...
                        device = device[fnName].apply(device, args[0])
         ...
         */
        client.selectById = function selectById(ids, callback, addressingOptions) {
            ...
            return client.call(function () {
                ...
                var newRemote = xdTesting.remote(newOptions, newMultiDevice.getModifier(addressingOptions), newMultiDevice, null);
