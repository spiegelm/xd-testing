    ...
    return function(client) {
        ...
        */
        ...
        client.forEach = (callback) => {
            ...
            //    results.push(new q(callback(multiDevice.instances[device], index++)));
            //});
            return client.call(function() {
                return q.all(Object.keys(multiDevice.instances).map((key, index) => callback(multiDevice.instances[key], index)));
                    //[callback(multiDevice.instances['A'], 0), callback(multiDevice.instances['B'], 1)]);
