...
app.get('/', function (req, res) {
    ...
    fs.access(STEPS_FILE, fs.F_OK, function(err) {
        ...
        let args = {
            ...
            //'deviceSteps': flow.devices().map(deviceId => {
            //    let deviceSteps = flow.deviceSteps(deviceId)
            //    return {deviceId: deviceId, deviceOptions: deviceSteps[0].deviceOptions, steps: deviceSteps}
            //}),
            //'steps': steps,
            //'checkpoints': flow.checkpoints
        };
