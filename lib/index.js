...
var remote = module.exports.remote = function remote(options, modifier, multiDevice) {
    ...
    let screenshotBlackList = {
        protocolQueries: ['element', 'elementActive', 'elementIdDisplayed', 'elementIdElement', 'elementIdElements', 'elementIdEnabled', 'elementIdLocation', 'elementIdLocationInView', 'elementIdName', 'elementIdSelected', 'elementIdSize', 'elementIdText', 'elements', 'windowHandle', 'windowHandles', 'title'],
    ...
    ['protocol', 'commands'].forEach(function(commandType) {
        ...
        files.forEach(function(filename) {
            ...
            let proxyCommandFunc = function () {
                ...
                let intermediate = this.saveScreenshot(
                    function(err, screenshot, response) {
                        ...
                        var step = new Step(localId, globalStep, commandName, screenshot, this.options);
            ...
            unit.lift(commandName,
                screenshotBlackList.indexOf(commandName) !== -1 ? commandFunc : proxyCommandFunc
            );
        });
    });
