...
class Flow {
    ...
    /**
     * Returns a grid of checkpoints. One column per device, one row per checkpoint.
     */
    grid() {
        ...
        // Do some initialization
        checkpoints.forEach((checkpoint) => {
            //if (! checkpointsWithName[checkpoint.name]) {
            //    checkpointsWithName[checkpoint.name] = []
            //}
            //checkpointsWithName[checkpoint.name].push(checkpoint)
            checkpointsWithId[checkpoint.id] = checkpoint
        })

        //console.log(checkpointsWithId)

        let uniqueCheckpoints = []

        checkpointsWithId.forEach(checkpoint => {
            // TODO verify this approach works for all cases
            // e.g. two devices working in parallel to two other devices
            if (
                checkpointsWithName.length > 0
                && checkpointsWithName[checkpointsWithName.length - 1].name == checkpoint.name
            ) {
                checkpointsWithName[checkpointsWithName.length - 1].checkpoints[deviceIndex[checkpoint.deviceId]] = checkpoint
            } else {
                let checkps = {}
                checkps[deviceIndex[checkpoint.deviceId]] = checkpoint
                checkpointsWithName.push({name: checkpoint.name, checkpoints: checkps})
            }
        })

        console.log(checkpointsWithName)
